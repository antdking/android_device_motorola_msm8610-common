...
export COMMON_NAME="msm8610-common"
...
if [ -f ../$COMMON_NAME/proprietary-files.txt ]; then
for FILE in `egrep -v '(^#|^$)' ../$COMMON_NAME/proprietary-files.txt`; do
    echo "Extracting /system/$FILE ..."
    OLDIFS=$IFS IFS=":" PARSING_ARRAY=($FILE) IFS=$OLDIFS
    FILE=${PARSING_ARRAY[0]}
    DEST=${PARSING_ARRAY[1]}
...
BASE=../../../vendor/$VENDOR/$COMMON_NAME/proprietary
rm -rf $BASE/*
for FILE in `egrep -v '(^#|^$)' ../$COMMON_NAME/common-proprietary-files.txt`; do
    echo "Extracting /system/$FILE ..."
    OLDIFS=$IFS IFS=":" PARSING_ARRAY=($FILE) IFS=$OLDIFS
    FILE=${PARSING_ARRAY[0]}
    DEST=${PARSING_ARRAY[1]}
...
../$COMMON_NAME/setup-makefiles.sh
